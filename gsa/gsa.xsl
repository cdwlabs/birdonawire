...
<!-- DCB -->
<xsl:variable name="spage">gsa.php?</xsl:variable>

<!-- *** choose result page header: '', 'provided', 'mine', or 'both' *** -->
<xsl:variable name="choose_result_page_header"></xsl:variable>

<!-- *** customize provided result page header *** -->
<xsl:variable name="show_swr_link"></xsl:variable>
<xsl:variable name="swr_search_anchor_text">Search Within Results</xsl:variable>
<xsl:variable name="show_result_page_adv_link"></xsl:variable>
...
<xsl:variable name="home_url"><xsl:value-of select="$spage" /><xsl:value-of select="$base_url"
...
<xsl:variable name="filter_url"><xsl:value-of select="$spage"></xsl:value-of><xsl:for-each 
...
<xsl:variable name="adv_search_url"><xsl:value-of select="$spage" /><xsl:value-of
...
<xsl:variable name="swr_search_url"><xsl:value-of select="$spage" /><xsl:value-of
...
<select name="site">
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='archive_collection')]">
   <option value="archive_collection" selected="selected">archive_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="archive_collection">archive_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='dc_wiki_collection')]">
   <option value="dc_wiki_collection" selected="selected">dc_wiki_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="dc_wiki_collection">dc_wiki_collection</option>
  </xsl:otherwise>
 </xsl:choose>
 <xsl:choose>
 ...
 <xsl:choose>
  <xsl:when test="PARAM[(@name='site') and (@value='ipcc_collection')]">
   <option value="ipcc_collection" selected="selected">ipcc_collection</option>
  </xsl:when>
  <xsl:otherwise>
   <option value="ipcc_collection">ipcc_collection</option>
  </xsl:otherwise>
 </xsl:choose>
</select>
...
<xsl:template name="sort_by">
  ...
  <xsl:choose>
    <xsl:when test="/GSP/PARAM[@name = 'sort' and starts-with(@value,'date:D:S')]">
      ...
      <a href="{$spage}{$sort_by_relevance_url}">Sort by relevance</a>
    </xsl:when>
    ...
    <xsl:otherwise>
      <a href="{$spage}{$sort_by_date_url}">Sort by date</a>
      <font color="{$global_text_color}">
...
<xsl:template name="spelling">
  <xsl:if test="/GSP/Spelling/Suggestion">
    <p><span class="p"><font color="{$spelling_text_color}">
       ...
       <a href="{$spage}q={/GSP/Spelling/Suggestion[1]/@qe}&amp;spell=1&amp;{$base_url}">
       <xsl:value-of disable-output-escaping="yes"
    ...
    <tr>
      <td class="s">
        ...
        <xsl:if test="HN">
          <br/>
          [
          <a class="f" href="{$spage}as_sitesearch={$crowded_url}&amp;{
...
<xsl:template match="C">
    ...
    <xsl:if test="$show_res_cache != '0'">
        ...
        <a class="f" href="{$spage}q=cache:{$docid}:{$escaped_url}+{
        ...
        <xsl:choose>
          <xsl:when test="$prev">
            <td nowrap="1">
              ...
              <a href="{$spage}{$search_url}&amp;start={$view_begin -
                      $num_results - 1}">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <td>
        ...
        <a href="{$spage}{$search_url}&amp;start={$start}">
        <xsl:if test="$navigation_style = 'google'">
